...
export interface DriveMockupProps {
  embedded?: boolean;
}

export const DriveMockup: React.FC<DriveMockupProps> = ({ embedded = false }) => {
  ...
  const handleSendQuestion = (questionText?: string) => {
    ...
    setChatMessages((prev) => [
      ...
      {
        sender: 'ai',
        text: `Based on your query regarding "${q}", the system analyzed 4 cross-referenced documents. All compliance requirements and SLA benchmarks are satisfied.`,
        citation: 'Verified against Workspace Security Policy v3.2',
      },
    ]);
    setChatInput('');
  };
