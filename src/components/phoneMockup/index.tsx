...
import BusinessHubApp from "./BusinessHubApp";
...
const PhoneMockup: React.FC<PhoneMockupProps> = ({ variant = "dashboard" }) => {
  ...
  const isBusinessHub =
    ...
    variant === "executive" ||
    variant === "org";
  const isAttendance = (variant === "attendance" || variant === "dark") && !isBusinessHub;
  ...
  const [liveIstTime, setLiveIstTime] = useState<string>("01:24:59 PM");
  ...
  const showDrive = !isEofficeVariant && activeTab === "drive";
  const showStream = !isEofficeVariant && (activeTab === "streamline" || activeTab === "stream");
  const showMail = !isEofficeVariant && activeTab === "mail";
  const showEoffice = isEofficeVariant || activeTab === "eoffice" || isAttendance;
  const showDashboard = !isEofficeVariant && !showDrive && !showStream && !showMail && !showEoffice;
    ...
    <div className={classes.frame}>
      ...
      <div className={`${classes.screen} ${isConnect ? classes.screenWhite : ''}`}>
        ...
        <div className={classes.statusBar}>
          <span className={classes.statusTime}>{isBusinessHub ? "1:04" : "3:39"}</span>
        ...
        {isBusinessHub ? (
          ...
          <>
            ...
            <div className={classes.screenContent}>
              ...
              ) : showEoffice || isAttendance || isEofficeVariant ? (
                /* ─────────────────────────────────────────────────────────────
                   DEDICATED E-OFFICE ATTENDANCE SCREEN (From User Screenshot)
                   ───────────────────────────────────────────────────────────── */
                <>
                  {/* App Bar */}
                  <div className={classes.appHeader}>
                    ...
                    <div className={classes.userAvatarCircle} title="Admin Profile">
                      <span>👨‍💼</span>
                    </div>
                  </div>

                  {/* Top Sub-Navigation (Attendance only; leave tab removed as instructed) */}
                  <div className={classes.eofficeSubNav}>
                    <button className={classes.eofficeSubNavTab}>Attendance</button>
                  </div>

                  {/* Top 3 KPI Metrics Row: Clock In, Clock Out, Total Hours */}
                  <div className={classes.kpi3GridMobile}>
                    {/* Clock In */}
                    <div className={classes.kpi3Card}>
                      <div className={classes.kpi3Header}>
                        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#0072C4" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                      ...
                      <div className={classes.kpi3Value}>{clockedIn ? "09:00 AM" : "—"}</div>
                      <button
                        className={`${classes.kpi3Btn} ${clockedIn ? classes.kpi3BtnActive : ""}`}
                    ...
                    <div className={classes.kpi3Card}>
                      <div className={classes.kpi3Header}>
                        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#9333EA" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                      ...
                      <div className={classes.kpi3Value}>—</div>
                    ...
                    <div className={classes.kpi3Card}>
                      ...
                      <div className={classes.kpi3Value}>{clockedIn ? "5:42" : "0:00"}</div>
                      <div className={classes.kpi3ProgressBar}>
                        <div
                          className={classes.kpi3ProgressFill}
                          style={{ width: clockedIn ? "71%" : "0%" }}
                        />
                      </div>
                    </div>
                  </div>

                  {/* IST Live Time Card (matching screenshot) */}
                  <div className={classes.timeCardLiveMobile}>
                    <div className={classes.timeCardZone}>
                    ...
                    <div className={classes.timeCardClockText}>{liveIstTime}</div>
                    <div className={classes.timeCardDatePill}>
                      <span>📅 21 September, 2026</span>
                    </div>
                  </div>

                  {/* Shift Details Card (matching screenshot) */}
                  <div className={classes.shiftDetailsCard}>
                    ...
                    <div className={classes.shiftMetricsGrid}>
                      <div className={classes.shiftMetricCol}>
                        ...
                        <span className={classes.shiftMetricValue}>9:00 AM</span>
                      </div>
                      <div className={classes.shiftMetricCol}>
                        <span className={classes.shiftMetricLabel}>Shift End</span>
                        <span className={classes.shiftMetricValue}>6:00 AM</span>
                      </div>
                      <div className={classes.shiftMetricCol}>
                        <span className={classes.shiftMetricLabel}>Lunch Time</span>
                        <span className={classes.shiftMetricValue}>01:00:00</span>
                      </div>
                      <div className={classes.shiftMetricCol}>
                        <span className={classes.shiftMetricLabel}>Eff. hours</span>
                        <span className={classes.shiftMetricValue}>21 hours</span>
                      </div>
                    </div>

                    {/* Shift Name & Weekdays */}
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span className={classes.shiftSubTitle}>Shift Name: Development</span>
                    </div>
                    <div className={classes.weekDaysRow}>
                      {["S", "M", "T", "W", "T", "F", "S"].map((day, idx) => {
                        const isWorkingDay = idx >= 1 && idx <= 5;
                        const isSelected = activeDay === idx;
                        return (
                          <div
                            key={idx}
                            className={isWorkingDay || isSelected ? classes.dayCircleActive : classes.dayCircle}
                    ...
                    <p className={classes.shiftFooterInfo}>
                      ...
                      <br />
                      <span className={classes.shiftFooterSub}>Break/Lunch Hours 01:00:00</span>
                    </p>
                  </div>

                  {/* Recent Activity Card (matching screenshot) */}
                  <div className={classes.activityCardMobile}>
                    <div className={classes.activityHeaderMobile}>
                      <h4 className={classes.activityTitleMobile}>Recent Activity</h4>
                      <div className={classes.activityToggleMobile}>
                        <button
                          className={`${classes.activityToggleBtn} ${activityPeriod === "week" ? classes.activityToggleBtnActive : ""}`}
                          onClick={() => setActivityPeriod("week")}
                        >
                          Week
                        </button>
                        <button
                          className={`${classes.activityToggleBtn} ${activityPeriod === "month" ? classes.activityToggleBtnActive : ""}`}
                    ...
                    <div className={classes.chartContainerMobile}>
                      <div className={classes.chartYAxisMobile}>
                        <span>24h</span>
                        <span>16h</span>
                        <span>8h</span>
                        <span>0h</span>
                      </div>
                      <svg className={classes.chartSvgMobile} viewBox="0 0 200 70">
                        {/* Grid Lines */}
                        <line x1="0" y1="10" x2="200" y2="10" stroke="#F1F5F9" strokeWidth="1" />
                        <line x1="0" y1="28" x2="200" y2="28" stroke="#F1F5F9" strokeWidth="1" />
                        <line x1="0" y1="46" x2="200" y2="46" stroke="#F1F5F9" strokeWidth="1" />
                        <line x1="0" y1="64" x2="200" y2="64" stroke="#E2E8F0" strokeWidth="1" />

                        {/* Trend Line (Mon at ~8h, Tue-Fri at 0h matching screenshot) */}
                        <polyline
                          fill="none"
                          stroke="#0072C4"
                          strokeWidth="2"
                          ...
                          points="20,54 60,64 100,64 140,64 180,64"
                        />
                        {/* Data points */}
                        <circle cx="20" cy="54" r="3" fill="#0072C4" />
                        <circle cx="60" cy="64" r="2.5" fill="#FFFFFF" stroke="#0072C4" strokeWidth="1.5" />
                        <circle cx="100" cy="64" r="2.5" fill="#FFFFFF" stroke="#0072C4" strokeWidth="1.5" />
                        <circle cx="140" cy="64" r="2.5" fill="#FFFFFF" stroke="#0072C4" strokeWidth="1.5" />
                        <circle cx="180" cy="64" r="2.5" fill="#FFFFFF" stroke="#0072C4" strokeWidth="1.5" />

                        {/* X-axis labels */}
                        <text x="20" y="69" fontSize="6.5" fill="#94A3B8" textAnchor="middle">Mon</text>
                        <text x="60" y="69" fontSize="6.5" fill="#94A3B8" textAnchor="middle">Tue</text>
                        <text x="100" y="69" fontSize="6.5" fill="#94A3B8" textAnchor="middle">Wed</text>
                        <text x="140" y="69" fontSize="6.5" fill="#94A3B8" textAnchor="middle">Thu</text>
                        <text x="180" y="69" fontSize="6.5" fill="#94A3B8" textAnchor="middle">Fri</text>
                      </svg>
                    </div>
                  </div>

                  {/* Teams Logs Card (matching screenshot) */}
                  <div className={classes.teamsLogCardMobile}>
                    <div className={classes.teamsLogLeftMobile}>
                      <div className={classes.teamsLogIconBoxMobile}>
                      ...
                      <div>
                        <h4 className={classes.teamsLogTitleMobile}>Teams Logs</h4>
                        <p className={classes.teamsLogSubMobile}>Review your direct report's attendance and clock in history</p>
                      </div>
                    </div>
                    <span style={{ fontSize: "14px", color: "#94A3B8", fontWeight: 700 }}>›</span>
                  </div>

                  {/* Log Section (matching screenshot log table) */}
                  <div className={classes.logSectionMobile}>
                    <h4 className={classes.logSectionTitleMobile}>Log</h4>

                    {/* Log Item 1: 2026-09-20 Weekend */}
                    <div className={classes.logCardItemMobile}>
                      <div className={classes.logCardRowTop}>
                        <span className={classes.logEmpName}>Admin</span>
                        <span className={`${classes.logStatusBadge} ${classes.badgeWeekend}`}>WEEKEND</span>
                      </div>
                      <div className={classes.logCardDate}>2026-09-20</div>
                      <div className={classes.logCardDetailsGrid}>
                        <span>Clock In: —</span>
                        <span>Clock Out: —</span>
                        <span>Gross: —</span>
                      </div>
                    ...
                    {/* Log Item 2: 2026-09-19 Weekend */}
                    <div className={classes.logCardItemMobile}>
                      <div className={classes.logCardRowTop}>
                        <span className={classes.logEmpName}>Admin</span>
                        <span className={`${classes.logStatusBadge} ${classes.badgeWeekend}`}>WEEKEND</span>
                      </div>
                      <div className={classes.logCardDate}>2026-09-19</div>
                      <div className={classes.logCardDetailsGrid}>
                        <span>Clock In: —</span>
                        <span>Clock Out: —</span>
                        <span>Gross: —</span>
                    ...
                    {/* Log Item 3: 2026-09-18 Late Arrival */}
                    <div className={`${classes.logCardItemMobile} ${classes.logCardItemLateMobile}`}>
                      <div className={classes.logCardRowTop}>
                        <span className={classes.logEmpName}>Admin</span>
                        <span className={`${classes.logStatusBadge} ${classes.badgeLate}`}>LATE ARRIVAL</span>
                      </div>
                      <div className={classes.logCardDate}>2026-09-18</div>
                      <div className={classes.logCardDetailsGrid}>
                        <span>In: 10:36:09</span>
                        <span>Out: 06:00:00</span>
                        <span>Gross: 00:00:00</span>
                      </div>
                    </div>

                    {/* Log Item 4: 2026-09-17 Late Arrival */}
                    <div className={`${classes.logCardItemMobile} ${classes.logCardItemLateMobile}`}>
                      <div className={classes.logCardRowTop}>
                        <span className={classes.logEmpName}>Admin</span>
                        <span className={`${classes.logStatusBadge} ${classes.badgeLate}`}>LATE ARRIVAL</span>
                      </div>
                      <div className={classes.logCardDate}>2026-09-17</div>
                      <div className={classes.logCardDetailsGrid}>
                        <span>In: 16:36:27</span>
                        <span>Out: —</span>
                        <span>Gross: 21:00:00</span>
                      </div>
                    </div>
                  </div>
                </>
